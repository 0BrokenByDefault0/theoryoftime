...
exports.TEMPO_MAP = exports.EUCLIDEAN_EXAMPLES = exports.DRUM_PATTERNS_BY_ID = exports.DRUM_PATTERNS = exports.DRUM_VOICE_LABELS = exports.DELAY_PRESETS = exports.METERS_BY_ID = exports.METERS = exports.NOTE_VALUES_BY_ID = exports.NOTE_VALUES = void 0;
exports.dotted = dotted;
exports.tuplet = tuplet;
exports.barLengthInBeats = barLengthInBeats;
exports.secondsPerBeat = secondsPerBeat;
exports.secondsPerBar = secondsPerBar;
exports.delayMs = delayMs;
exports.swingOffset = swingOffset;
exports.euclideanRhythm = euclideanRhythm;
exports.polyrhythmGrid = polyrhythmGrid;
exports.tempoLabel = tempoLabel;
exports.NOTE_VALUES = [
...
exports.NOTE_VALUES_BY_ID = Object.fromEntries(exports.NOTE_VALUES.map((v) => [v.id, v]));
/** A dot adds half the note's value again; two dots add a further quarter. */
function dotted(beats, dots = 1) {
    let total = beats;
    let add = beats;
...
function tuplet(beats, inTheSpaceOf, playing) {
...
exports.METERS_BY_ID = Object.fromEntries(exports.METERS.map((m) => [m.id, m]));
/** Length of one bar in beats (quarter-note beats). */
function barLengthInBeats(sig) {
...
function secondsPerBar(bpm, sig) {
 ...
 */
function delayMs(bpm, beats) {
    return (60000 / bpm) * beats;
}
exports.DELAY_PRESETS = [
...
const P = (id, name, genre, bpm, note, tracks, swing) => ({
    id,
    name,
    genre,
    bpm,
    ...
    note,
    swing,
    tracks: tracks.map(([voice, hits]) => ({ id: voice, label: exports.DRUM_VOICE_LABELS[voice], hits })),
});
exports.DRUM_PATTERNS = [
    P('four-on-floor', 'Four on the Floor', 'House / Disco', 124, 'A kick on every beat gives the body something unmissable to lock to. Open hats on the off-beats create the forward lean.', [['kick', [0, 4, 8, 12]], ['clap', [4, 12]], ['hat', [2, 6, 10, 14]], ['openhat', [2, 6, 10, 14]]]),
    P('backbeat', 'Rock Backbeat', 'Rock / Pop', 120, 'Kick on 1 and 3, snare on 2 and 4. The snare on the weak beats is what makes it feel like popular music rather than a march.', [['kick', [0, 8]], ['snare', [4, 12]], ['hat', [0, 2, 4, 6, 8, 10, 12, 14]]]),
    P('boom-bap', 'Boom Bap', 'Hip Hop', 90, 'Slightly behind-the-beat snare and a swung hat grid. The looseness is the point — perfectly quantised boom bap sounds dead.', [['kick', [0, 6, 10]], ['snare', [4, 12]], ['hat', [0, 2, 4, 6, 8, 10, 12, 14]]], 0.55),
    P('trap', 'Trap', 'Trap', 140, 'Half-time feel: the snare lands only on beat 3, so the tempo reads as half. Hat rolls fill the space that leaves.', [['kick', [0, 3, 10]], ['snare', [8]], ['hat', [0, 2, 4, 6, 7, 8, 10, 12, 13, 14, 15]]]),
    P('amen', 'Amen-Style Break', 'Drum & Bass', 174, 'Syncopated ghost kicks and snares displaced off the grid. Density plus displacement is what creates the sense of speed.', [['kick', [0, 10]], ['snare', [4, 7, 12, 14]], ['hat', [2, 6, 10, 14]]]),
    P('bossa', 'Bossa Nova', 'Latin / Jazz', 130, 'The rim pattern is a two-bar clave-derived figure. Nothing lands squarely, which is why it floats.', [['kick', [0, 6, 8, 14]], ['rim', [0, 3, 6, 10, 12]], ['hat', [0, 2, 4, 6, 8, 10, 12, 14]]]),
    P('son-clave', 'Son Clave (3-2)', 'Afro-Cuban', 100, 'Five strokes across two bars that organise everything else in the arrangement. The reference the whole band feels against.', [['perc', [0, 3, 6, 10, 12]], ['kick', [0, 6, 8, 14]]]),
    P('dembow', 'Dembow', 'Reggaeton', 96, 'The "boom-ch-boom-chick" figure. Kick on the downbeat, snare on the "and" of 1 and on 2 — endlessly repeating.', [['kick', [0, 6, 8, 14]], ['snare', [3, 4, 11, 12]], ['hat', [0, 4, 8, 12]]]),
    P('shuffle', 'Blues Shuffle', 'Blues', 100, 'Every eighth becomes a triplet. Long–short, long–short. Straighten it and the blues disappears.', [['kick', [0, 8]], ['snare', [4, 12]], ['hat', [0, 2, 4, 6, 8, 10, 12, 14]]], 1),
    P('breakbeat', 'Breakbeat', 'Breaks / Big Beat', 132, 'Kick displaced off the downbeat and syncopated snares. It swings without swing, purely through placement.', [['kick', [0, 3, 8, 11]], ['snare', [4, 12]], ['hat', [2, 6, 10, 14]]]),
    P('garage', 'UK Garage 2-Step', 'UK Garage', 132, 'The kick skips beat 3 entirely and the snare shuffles. Removing an expected hit is as powerful as adding one.', [['kick', [0, 10]], ['snare', [4, 12]], ['hat', [2, 5, 6, 9, 13, 14]]], 0.6),
    P('afrobeats', 'Afrobeats', 'Afrobeats', 104, 'A three-against-four log-drum pulse over a steady kick. The tension between the two grids is the groove.', [['kick', [0, 6, 11]], ['perc', [0, 3, 6, 9, 12, 15]], ['hat', [2, 4, 6, 10, 12, 14]]]),
];
exports.DRUM_PATTERNS_BY_ID = Object.fromEntries(exports.DRUM_PATTERNS.map((p) => [p.id, p]));
/**
 ...
 */
function euclideanRhythm(pulses, steps, rotation = 0) {
    if (pulses <= 0)
...
function tempoLabel(bpm) {
    const found = exports.TEMPO_MAP.find((t) => bpm >= t.range[0] && bpm < t.range[1]);
